...
const EditBlog = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        setLoading(true);
        const res = await blogService.getMyBlogs({ limit: 100 });
  ...
  const onSubmit = async (data, isDraft = false) => {
    ...
    try {
      ...
      toast.success(isDraft ? 'Draft saved' : 'Blog updated successfully');
      navigate('/my-blogs');
    } catch (err) {
  ...
  return (
    <>
      <Helmet>
        <title>Edit Blog - BlogNest</title>
      </Helmet>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <form className="rounded-2xl p-6 sm:p-8 space-y-6" style={{ backgroundColor: 'var(--bg-secondary)' }}>
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Excerpt</label>
              <textarea
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Featured Image</label>
              <ImageUpload
                file={featuredImage}
